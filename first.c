...
int main(int argc, char **argv) {
    ...
    for (; countIters < MAX_ITERATIONS; ++countIters) {
        ...
        if (prevParam <= endParam) { // условие смены знака скаляра.
            if (useTau)
                break; // Очевидно, что на прямой к числу можно приближаться либо слева, либо справа
            tau *= -1;
            useTau = 1;
        }
        memcpy(x_n, x, N * sizeof(double)); // swap
        if (endParam < EPSILON_SQUARE || endParam == DBL_MAX) { // Условия выхода из спецификации
            break;
        }
        prevParam = endParam;
    }
